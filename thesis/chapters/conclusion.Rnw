...
\Sexpr{floor(d$img_cpu[surf]/d$img_cpu[orb])} 
compared with SURF. ORB is interesting for real-time applications.  Oriented
BRIEF is one of the first feature detectors and descriptors that permit to
...
the success rate and the average guess score were not impacted by trading
exactness for speed.
...
speed in our experiments. We demonstrated that Oriented BRIEF can be used
...
An idea would be to take the uncertainty in the correspondences into
