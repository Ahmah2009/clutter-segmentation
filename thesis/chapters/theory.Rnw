...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.8\textwidth]{media/multi-view-or}
...
brighter or darker pixels \cite{Bay2006}. Third, there are {\it region
detectors}, such as MSER, which extract features by finding image regions.
