...
halfway between the two chessboards. Yet, we need the object-camera
transformation $\ocT$. The solution here consists in just {\it defining} the
object coordinate system to be the same as the coordinate system attached to
the fiducial markers, that is
\begin{equation}
    \ocT: \real^3 \to \real^3, \vec{p} \mapsto\ \fTv(\vec{p})
...
object in terms of a object-camera transformation and $S \subseteq C_i$ the
...
ground truth pose $\ocT$ and pose estimate $\ocTh$ is given by

\begin{equation}
    e_t = \normtwo{\ \ocTh \pth{ \vec{0} -\ \ocT \vec{0} }\ }
\end{equation}
