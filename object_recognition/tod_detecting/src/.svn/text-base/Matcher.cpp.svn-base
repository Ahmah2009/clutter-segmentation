...
Matcher* Matcher::create(MatcherParameters& params)
{
  ...
  {
    matcher = new BruteForceMatcher<L1<float> > ();
  }
  ...
  {
    LshMatcher* lsh_matcher = new LshMatcher();
    lsh_matcher->setDimensions(32, 8, 13);
    matcher = lsh_matcher;
  }
  else
  {
...
Matcher::Matcher(const Ptr<DescriptorMatcher> matcher, int knn):matcher(matcher),knn_(knn)
{
}
...
void Matcher::add(const TrainingBase& base)
{
  ...
  {
    matcher->add(base.getObject(objInd)->getDescriptors());

    int prevSize = 0;
    if (objInd)
      prevSize = objectSizes[objInd - 1];
    objectSizes[objInd] = base.getObject(objInd)->observations.size() + prevSize;
  }
...
void Matcher::privateMatch(const Mat& queryDescriptors, tod::Matches& matches)
{
  ...
  std::sort(matches.begin(), matches.end()); //keep the matches sorted by distance
  matches.resize(matches.size() * 0.50); //take the top 50 percent
...
void RatioTestMatcher::match(const Mat& queryDescriptors)
{
  ...
  for (int descInd = 0; descInd < queryDescriptors.rows; descInd++)
  {
    ...
    indices.resize(objectIds.size());

    if (knnMatches[descInd].empty())
      continue;

    for (int kInd = 0; kInd < knnNum; kInd++)
    {
      int objIndex = getObjectIndex(knnMatches[descInd][kInd]);
      if (indices[objIndex].size() < 2)
      {
        indices[objIndex].push_back(kInd);
      }
    }

    for (size_t objInd = 0; objInd < objectIds.size(); objInd++)
    {
      int size = indices[objInd].size();
      if (size)
      {
        int firstI, secondI;
        firstI = indices[objInd][0];
        if (size == 2)
        {
          secondI = indices[objInd][1];
        }
        else
        	{
        	if (firstI < knnNum - 1)
        ...
        if (knnMatches[descInd][firstI].distance*knnMatches[descInd][firstI].distance < ratioThreshold * knnMatches[descInd][secondI].distance*knnMatches[descInd][secondI].distance)
        {
          DMatch match = knnMatches[descInd][firstI];
          if (objInd)
            match.imgIdx -= objectSizes[objInd - 1];
          objectMatches[objInd].push_back(match);
        }
      }
    }
  }
...
FlannRatioTestMatcher::FlannRatioTestMatcher(int knnNum_, double ratioThreshold_): knnNum(knnNum_), ratioThreshold(ratioThreshold_)
{
  flann_index = NULL;
}
...
void FlannRatioTestMatcher::match(const Mat& queryDescriptors)
{
  ...
  for (int i = 0; i < m_indices.rows; ++i)
  {
    for (size_t objId = 0; objId < objectIds.size(); objId++)
    {
      ...
      if (dists_ptr[knnNum * i + first_index] < ratioThreshold * dists_ptr[knnNum * i
          + second_index])
      {
        DMatch match;
        match.queryIdx = i;
        match.trainIdx = keypointIndices[indices_ptr[i * knnNum]];
        match.imgIdx = imageIndices[indices_ptr[i * knnNum]];
        objectMatches[objectIndices[indices_ptr[i * knnNum]]].push_back(match);
      }
    }
  }
}
