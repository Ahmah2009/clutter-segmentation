...
by either LSH or brute-force matching does not need to match. LSH ran at least
a factor \Sexpr{floor(sps_bf_avg$avg_test_runtime / sps_avg$avg_test_runtime)}
faster than brute-force search. Both LSH and brute-force search lead to similar
success rates.

%TODO: give exact number 81% and 82% but first need to see whether 
% vlsam initializes the seed for generating random numbers
...
which probably makes it hard to obtain informative features. Furthermore,
