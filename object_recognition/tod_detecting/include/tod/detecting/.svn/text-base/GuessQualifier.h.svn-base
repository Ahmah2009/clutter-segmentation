...
namespace tod
{
  ...
  private:
    ...
    void initPointsVector(std::vector<cv::Point2f>& imagePoints, std::vector<cv::Point3f>& objectPoints, int imageIdx);
    void clarifyGuess(Guess& guess, std::vector<cv::Point2f>& imagePoints, std::vector<cv::Point3f>& objectPoints);
  };
}
