...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{The modelbase with four different template objects used in evaluation}
    \end{center}
\end{figure}
...
long as they do not exceed the maximum projection error which is a circle on
the image plane. The problem can be traced back to the fact objects that are
far away from the camera merely degenerate to a point on the image plane. All
correspondences within the circle are designated inliers.
...
\begin{figure}[h]
    \begin{center}
        ...
        \caption{Far-away pose estimate can result in many inliers}
    \end{center}
\end{figure}
...
\begin{figure}[h]
    \begin{center}
        ...
        \caption{On a test scene, the badly aligned pose still results in many inliers}
    \end{center}
\end{figure}
