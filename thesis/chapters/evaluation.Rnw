...
<<>>=
    ...
    barplot(d$kpt_cpu, names.arg=d$extractor, main="CPU Time per keypoint", ylab="time [ms]", ylim=c(0, 0.5), col=col)
    barplot(d$quantity, names.arg=d$extractor, main="Keypoints per image", ylab="quantity", ylim=c(0, 2000), col=col)
    invisible(dev.off())
@
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{lll}
        ...
        OS & Ubuntu 10.10 Maverick \\
    \end{tabular}
    \caption{Hard- and software configuration for feature detector and descriptor benchmark}
...
and scene, \clutseg estimated a distance of 88 metres between \icedtea and
