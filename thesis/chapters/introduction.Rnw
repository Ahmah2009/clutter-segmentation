...
The system presented in this work enables a Personal Robot 2 (PR2,
http://www.willowgarage.com/pages/pr2/overview) to detect an object and
...
this work \cite{Rublee2011}.  Matching observed features with model features is
a common sub-task in object recognition. 
...
of the robot. It simultaneously provides color images and depth measurements. 
...
features are extracted from the grayscale image.  \refFigure{figure:keypoints}
shows both an image of a textured object (a) and the keypoints extracted from a
grayscale version of the image (b). The local 2D features, together with the
