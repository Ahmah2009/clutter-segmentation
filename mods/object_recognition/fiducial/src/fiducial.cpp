...
{
    std::cout << "------------------------------------------------------------------" << endl;
    std::cout << "Estimating pose on " << current_image_name << endl;
  ...
  if (observation.channels() == 1)
      cvtColor(observation, canvas, CV_GRAY2BGR);
  else
      observation.copyTo(canvas);

  if (all_object_pts.empty()) {
    std::cerr << "[FIDUCIAL] all_object_pts.empty() => true" << std::endl;
    
    fiducial_.draw(canvas, corners, found);
    imwrite(current_image_name + ".pose.png", canvas);
    std::cout << "Writing " << current_image_name + ".pose.png" << endl;
  ...
  } else
  {
    ...
    if (true || verbose_)
    {
      ...
      waitKey(20);
      imwrite(current_image_name + ".pose.png", drawImage);
        std::cout << "Writing " << current_image_name + ".pose.png" << endl;
    }
    return pose;
  }
