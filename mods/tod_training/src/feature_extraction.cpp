...
FeatureDetector* FeatureExtractor::createDetector(const string& detectorType, float threshold)
{
  ...
  {
    fd = new StarFeatureDetector(16/*max_size*/, (int)threshold/*response_threshold*/, 10/*line_threshold_projected*/,
...
FeatureExtractor* FeatureExtractor::create(FeatureExtractionParams params)
{
  ...
  if ((params.extractor_type == "ORB") || (params.detector_type == "ORB") || (params.descriptor_type == "ORB"))
  {
  ...
  {
    detector = new DynamicAdaptedFeatureDetector(new rbrief::RBriefFastAdjuster(80, true, 10, 255), min_features,
                                                 max_features, 200);
  }
  ...
  if (params.descriptor_type == "rBRIEF")
  {
    ...
    rbrief_extractor->setPatchSize(rbrief::RBriefDescriptorExtractor::PATCH_LEARNED);
    // Set the steerer
  ...
  else if (params.extractor_type == "ORB")
  {
    ...
    {
      fe = new OrbExtractor(1.2, params.extractor_params["octaves"], max_features,
                            rbrief::RBriefDescriptorExtractor::PATCH_LEARNED, true, OrbExtractor::ANMSFast);
    }
    else
    {
      fe = new OrbExtractor(1.2, params.extractor_params["octaves"], max_features,
                            rbrief::RBriefDescriptorExtractor::PATCH_LEARNED, true, OrbExtractor::HarrisFast);
    }
  }
  return fe;
...
void MultiscaleExtractor::detectAndExtract(Features2d& features) const
{
  ...
  for (int i = 0; i < octaves; i++)
  {
    ...
    Mat
        n_desc(features.descriptors.rows + descriptors.rows, extractor_->descriptorSize(), extractor_->descriptorType());
...
OrbExtractor::OrbExtractor(float scale_factor, int n_levels, int n_desired_features,
                           rbrief::RBriefDescriptorExtractor::PatchSize ps, bool steering, DetectorType detector) :
  scale_factor_(scale_factor), n_levels_(n_levels > 0 ? n_levels : 1), n_desired_features_(n_desired_features),
      ...
      rbriefs_(n_levels_), get_patches_(false)
{
    ...
    switch (detector)
    {
      case ANMSFast:
        feature_detectors_.push_back(
                                     new rbrief::ANMSFastDetector(
                                                                  n_desired_features_
                                                                      * (1 / std::pow(scale_factor_, i))));
        break;
      case HarrisFast:
        feature_detectors_.push_back(
                                     new rbrief::SimpleFASTHarris(
                                                                  n_desired_features_
                                                                      * (1 / std::pow(scale_factor_, i)), true));
        break;
      case Fast:
...
void GetPatches(cv::Size patch_size, const cv::Mat& image, const std::vector<cv::KeyPoint>& kpts,
  ...
  BOOST_FOREACH(const cv::KeyPoint& kpt, kpts)
        {
          ...
          cv::Mat w = GetAffineRotationMat(0, t2) * GetAffineRotationMat(-kpt.angle, Point())
...
void OrbExtractor::detectAndExtract(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors,
                                    const cv::Mat& mask) const
{
  ...
  for (int i = 0; i < n_levels_; i++)
  {
    if (i != zoom_level)
    {
      ...
      scales[i] = scale;
      cv::resize(working_mats_[zoom_level], working_mats_[i], cv::Size(), scale, scale, cv::INTER_AREA);
      if (!mask.empty())
        cv::resize(working_masks_[zoom_level], working_masks_[i], cv::Size(), scale, scale, cv::INTER_AREA);
