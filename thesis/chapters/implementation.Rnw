...
the 3D point cloud, together forming the sought-after depth images. These
images and other data can be recorded in so-called bags. \ros contains the
\vslam package, which is only used by \tod because it contains a RANSAC
implementation for solving the PnP problem.

The {\it Point Cloud Library} (\pcl) is concerned with processing 3D data. A
...
consist of a depth image and a grayscale image each. The views are the basic
units processed in a pipeline that comprises three stages for each view: First,
the computation of the object-view transformation from the image.  Second, the
segmentation of the region of interest that shows the object on the image.
Third, the extraction of local 2D features from the grayscale image, followed
by the construction of model features from the region of interest.

\subsubsection{Camera Calibration}

Given a view, the first step is to obtain the object-view transformation by
estimating the extrinsic parameters of the camera. The camera calibration is
based on chessboard-like fiducial markers that are glued onto the rotating
table on two opposite sides. \opencv offers camera calibration methods that
help to find the chessboard corners and then estimate the extrinsic parameters
by solving the perspective-n-point problem. 

\begin{figure}[h!] \begin{center}
\includegraphics[width=0.5\textwidth]{fiducial-object} \caption{The
...
on the image at all, we used dithered binary images as a fallback if the
chessboard corner detection on the original images failed. This improved
reliability. We singled out 23 cases of the original images where camera
calibration failed. Some tested image processing operations work better than
others on these 23 images (\refTable{table:camera-calib-fallback}). Three
pre-processed images are shown in \refFigure{figure:camera-calib-fallback}.
...
\begin{table}[h!]
    ...
    \caption[Various image processing operations for more reliable camera calibration]{Success rates of detecting chessboards after image processing
...
to be aligned as close as possible to the true region of interest. Due to some
bug in the underlying software, the estimate for the region of interest
...
disk of $2.3$ to compensate for the initial opening step.
...
uniquely associates the keypoint $\vec{p'}$ with a 3D point, described in view
coordinates $\vec{p_v}$ in the scene. Applying the inverse object-view
transformation, the object coordinates $\vec{p}$ are obtained by $\vec{p} =\
\vTo \pth{\vec{p_v}}$. This yields a model feature $(\vec{p}, \vec{d})$.
\tod stores all learned models in a {\it modelbase}.

In the actual implementation, \tod applies the inverse object-view
...
% MAYBE: test what happens if one uses just every second, third, or fourth view
% it is no longer (see changeset https://code.ros.org/trac/wg-ros-pkg/changeset/50688/branches/trunk_diamondback/stacks/object_recognition/tod_detecting/src/Loader.cpp)
% that tod* select the views by angle steps.
...
{\tt knn}.  \tod implements the ratio test, as described in \cite{1999}.
...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.55\textwidth]{inlier-criteria.png}
