...
<<fig=true,png=true,height=4, width=10>>=
    ...
    plot(xy$x, xy$y, type="lines", xlab="Experiment identifier", ylab = "Success rate")
    hist(r_all$succ_rate, xlab = "Success rate", main="")
@
    \end{center}
...
\begin{table}[h!]
  \begin{center}
    ...
    \end{tabular}
    \caption[Selected \clutseg parameters for feature extraction]{Selected
        \clutseg parameters for feature extraction.}
...
\begin{table}[h!]
  \begin{center}
    ...
    \end{tabular}
    \caption[Selected \clutseg parameters for feature matching and pose
            estimation]{Selected \clutseg parameters for feature matching and pose
            estimation.}
