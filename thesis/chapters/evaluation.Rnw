...
required raw data, which has been collected in a similar fashion, using the
...
In ROS, distributed nodes communicate by publishing and subscribing to topics.
In our case, the interesting topics were the image and the 3D point cloud,
together forming the sought-after depth images. In ROS, depth images and other
data can be recorded in so-called bags.
...
described in \refSection{subsection:learning}.  This post-processed raw data
provides the masks and the object-view transformation which can be used to
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption[2D view of objects used in evaluation]{The four objects used in evaluation}
...
% FIXME: cannot show the models, move to issues!
...
% TODO: fix OpenCV-ORB <=> ORB naming issue
% FIXME: fix selected parameter set
...
experiments. This knowledge is required for interpreting data from multiple
...
set. It also covers observations made on a broad range of experiments.
...
\clutseg achieved an average scene score of
...
\begin{figure}[h]
    \begin{center}
        ...
        \caption[Validation scene with successfully recognized \icedtea]{From left to right: (a) A validation scene; (b) a visualization of the estimated pose for \icedtea}
        \label{figure:validation-scene-success}
    \end{center}
\end{figure}

A pose estimate returned by \clutseg was supported by
\Sexpr{round(sps_avg$avg_refine_choice_inliers)} correspondences on average.

% TODO: averaging

The success rate is over-optimistic with regard to the test scenes, though,
...
@
     ...
     \caption[Comparison of SIFT, SURF and ORB in terms of speed]
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{lll}
        ...
        OS & Ubuntu 10.10 Maverick \\
    \end{tabular}
    \caption{Hard- and software configuration for computation speed benchmark}
...
Sometimes it is necessary to control the number of features selected by a
feature detector.  In general, the number of features selected by a feature
detector should be configurable, using a simple and intuitive threshold
\cite{Tuytelaars2007}.

A parameter {\tt n\_features} specifies the number of desired features produced
by Oriented BRIEF. Other feature detectors and descriptors in \opencv, such as
...
on the image. The parameter is just a hint. The question arises how
well ORB responds to different numbers of desired features.
...
<<>>=
    ...
    invisible(dev.off())
@
     \includegraphics[width=0.6\textwidth]{orb-response-n-features}
     \caption{ORB response on number of desired features}
...
carry different amount of informations, though all were sampled at the same
resolution of 512x512 pixels. Increasing values for {\tt n\_features} have
been tried until the number of actual keypoints reaches its maximum for all
four images.

{\it Mandrill} has very fine-grained texture, and ORB extracted maximally
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption[Repetitive texture on template object]{Repetitive texture on template object \icedtea }
    \end{center}
\end{figure}
...
(\refFigure{figure:distant-pose-estimate-scheme}). For the $n$ correspondences
...
expected. The effect can be neglected if radius $r$ is very small.  In the
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.55\textwidth]{media/ransac-max-projection-error-deficiency}
        \caption[Schematic explanation of distant pose estimate with many inliers]{A far-away pose estimate can result in many inliers}
        \label{figure:distant-pose-estimate-scheme}
    \end{center}
\end{figure}
...
\subsection{Systematic Error in Model}
...
\begin{figure}[h]
    \begin{center}
        ...
        \caption{From left to right: (a) Correct model of {\it downy}; (b) side view of {\icedtea} model; (c) top view of {\icedtea} model}
        \label{figure:systematic-error-model}
    \end{center}
\end{figure}

In order to prove that our models got corrupted by some systematic error,
\refFigure{figure:clutseg-model-xyhist} shows the {\it $x$-$y$-histograms} of
the $x,y$-coordinates of the model points. These histograms were computed by
projecting the model points orthogonally onto the $x,y$-plane, counting the
number of points that fall into the same bin of 320x320 bins (pixels) in total.
Outliers have not been included in the histogram. Especially \icedtea's
$x$-$y$-histogram has roughly the form of a hash ('\#'). 

\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{media/clutseg-model-xyhist}
        \caption[X-Y-Histograms of \clutseg models]{From left to right: xy-histogram (a) of \assamTea, (b) of \haltbareMilch, (c) of \jacobsCoffee, (d) of \icedtea}
        \label{figure:clutseg-model-xyhist}
    \end{center}
\end{figure}

In comparison, the \tod model histograms are as expected
(\refFigure{figure:tod-model-xyhist}). Even the texture-less lid on the top of
\fatFreeMilk can be seen on the histogram (\refFigure{figure:tod-model-xyhist} c).  Just like \icedtea, \fatFreeMilk is a cuboid. Hence, their
$x$-$y$-histograms should be similar, which is not the case.
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{media/tod-model-xyhist}
        \caption[X-Y-Histograms of \tod models]{From left to right: xy-histogram (a) of {\it campbells\_chicken\_noodle}, (b) of {\it downy}, (c) of {\it fat\_free\_milk}, (d) of {\it good\_earth\_tea}}
        \label{figure:tod-model-xyhist}
    \end{center}
\end{figure}

Unfortunately, the source for the systematic error is yet unknown, although the
odds are high that it is related to camera calibration and accuracy of the
depth image.
