...
rotation matrix $R \in \mathbb{R}^3$ and a translation vector $\mathbf{t} \in
\mathbb{R}^3$.

\begin{equation}
    T: \mathbb{R}^3 \to \mathbb{R}^3, \mathbf{v} \mapsto R\mathbf{v} + \mathbf{t}
\end{equation}

A proper rigid transformation, we abbreviate it by PRT in the following, can be
...
\begin{equation}
    T^{-1}: \mathbb{R}^3 \to \mathbb{R}^3, \mathbf{v} \mapsto R^{-1}(\mathbf{v} - \mathbf{t})
\end{equation}
...
Let's review a few examples to show the usefulness of these definitions. If we
like to know the origin of the object in view coordinates, we can apply the
object-view transformation to the zero vector, and can compute the
distance $d$ between camera and object origin as

$$ d = \left|\left|\ _{xyz}T^{uvw}\left[ \mathbf{0} \right]\ \right|\right|_2$$

Another example, we can compute the distance $d_{12}$ between the two objects
in a scene by

$$ d_{12} = \left|\left|\ _{xyz}T_1^{uvw}\left[ \mathbf{0} \right] -
                  _{xyz}T_2^{uvw}\left[ \mathbf{0} \right]\ \right|\right|_2$$
...
space, three for translation and three for orientation. Three points in a 2-d
perspective projection are necessary and sufficient to determine a pose. If
there were no noise in the set of 2-d correspondences, we would be able to find
poses of an object by simply taking any three correspondences belonging to this
object. Unfortunately, image noise, discretization effects and approximate
...
to compute the projection error and the consensus set. The size of the resulting
consensus sets plays an important role in this work.
...
Classification is about making decisions, given some observations, usually in
the presence of uncertainty. In classification, we choose from a finite number
of possible decisions. 

\subsection{Estimation}

Estimation is about estimating parameters or other continuous numeric vectors,
given some observations. It can be regarded as the continuous extension of
classification. 

\subsection{Recognition}

The recognition problem, where objects have to be both correctly labeled and
its location determined, neither fits to a pure classification nor to a pure
estimation problem.
