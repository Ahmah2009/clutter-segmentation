...
the associated 3D points on the object's surface, form {\it model features} and
make up the the model of this object. The model is a sparse point cloud and is
described in a standard coordinate system.
...
search permits to find correspondences between these query features and the
model features. Based on these correspondences, a model-fitting algorithm that is
