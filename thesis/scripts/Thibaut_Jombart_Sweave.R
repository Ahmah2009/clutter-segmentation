             ...
             stylepath, ...)
{
    ...
    options <- list(prefix=TRUE, prefix.string=prefix.string,
                    ...
                    fig=FALSE, pdf=TRUE, eps=TRUE, png=FALSE,
                    width=6, height=6, center=TRUE, term=TRUE,
    ...
    RweaveLatexRuncode <- function(object, chunk, options)
	  ...
	  thisline <- 0
          ...
          if(options$fig && options$eval){
              ...
              if(options$png){
                  png.width <- ifelse(is.null(options$width), 480, options$width * 80) # can't pass width=NULL to png
                  png.height <- ifelse(is.null(options$height), 480, options$height * 80) # can't pass height=NULL to png
            ...
            if (inherits(err, "try-error"))
              ...
              if(options$include) {
                  ...
                  if(!is.null(options$size)){
                      cat("\\resizebox{", options$size,"\\textwidth}{!}{\\includegraphics{",
                          chunkprefix, "}}\n", sep = "", file = object$output, append = TRUE)
                  } else {
...
RweaveLatexFinish <- function(object, error=FALSE)
{
    ...
    if(!object$quiet && !error)
        cat("\n",
            gettextf("You can now run LaTeX/pdfLaTeX on '%s'", outputname),
            "\n", sep = "")
    close(object$output)
