...
review the required mathematical tools and the basics 3D object recognition
with models learnt from multiple views. We discuss Oriented BRIEF, a new
feature detector and descriptor. We refer to approximative nearest neighbor
search, which permits to quickly establish correspondences between known and
unseen images. Then, we show how pose estimation is related to other problems,
and how to solve it in the presence of noise and mismatches in the correspondences.
Finally, this chapter shows how the implemented system relates to the framework
...
coordinate system is defined by the depth camera used to assemble the model.
In this section, we explain how a coordinate system attached to the camera and
a coordinate system attached to the object relate by proper rigid
transformations. We precisely define the pose of an object with respect to the
camera in terms of a proper rigid transformation. We also describe how pose
estimation is related to camera calibration and the perspective-n-point
problem.
...
A {\it rigid transformation} is defined as an orthogonal transformation
followed by a translation in Euclidean space. An orthogonal transformation is
either a reflection or a rotation. A {\it proper rigid transformation}
additionally requires the orthogonal transformation to be a rotation
\footnote{The definitions of rigid transformations found in the literature are
inconsistent. Reflections are excluded from rigid transformations in
\cite{Forsyth2003}, but included in \cite{Galarza2007}.} . In three dimensions,
a proper rigid transformation of vectors can be represented by a rotation
matrix $R \in \real^{3\times3}$ and a translation vector $\vec{t} \in \real^3$.
...
system. In physics, this is called an active transformation, and explains the
...
system. This gives rise to the question of what coordinate systems are
convenient and how many are necessary. In our case, the two different
coordinate systems discussed in the following prove sufficient.

The {\it view coordinate system} is attached to the camera. Two of its basis
...
to know the view coordinates of the two grasping points. The {\it object
coordinate system} is attached to the object. Both coordinate systems are shown
in \refFigure{figure:coordinate-frames}. The PRT between object coordinates and
view coordinates is called the {\it object-view transformation} and is denoted
by $\oTv$. Subsequently, its inverse PRT is called {\it view-object
...
\begin{figure}
    ...
    \end{center}
    \caption{Object coordinate system and view coordinate system}
...
orientation of the object with respect to the camera, we simplify things by
assuming the camera to be fixed and by always looking at scenes from the eyes
of the robot. Then, it is possible to restrict ourselves to the study of rigid
motions of objects, which are equivalent to the change of object-view
transformations. Note that if there are multiple objects in a scene, then there
exists a object-view transformation for each object. For example, if there are
two objects $1$ and $2$ in the scene, the distance $d_{12}$ between them may be
computed by
...
We can reformulate the problem of recognizing and estimating the pose of several
objects in a scene as

\begin{verbatim}
    Estimate the object-view transformation for each
    of the objects in the scene.
\end{verbatim}

In the weakened case of only one object

\begin{verbatim}
    Estimate the object-view transformation for one
    of the objects in the scene.
\end{verbatim}
...
shown in \refFigure{figure:hm-features-model}. In the recognition process,
...
about the nature of the relationship between the scene and its 2D image, which
...
perspective projection is used to determine the projection of a 3D point on the
image plane \cite{Bradski2008}. The image plane, unlike the original pinhole
camera, is conveniently defined in front of the center of perspective
\cite{Forsyth2003, Bradski2008}. 
...
coordinate system in its environment.

% TODO: refer to problem definition in previous section

\begin{verbatim}
    For each object in the scene, estimate the extrinsic camera
    parameters with respect to its object coordinate system. 
\end{verbatim}

Estimation of camera parameters belongs to the topic of {\it camera
calibration}.  In fact, \tod uses camera calibration techniques from OpenCV,
where the intrinsic parameters are known, but the extrinsic parameters remain
to be estimated.
...
can be estimated. This is the perspective-$n$-point problem (PnP)
\cite{Fischler1981}.
...
Lowe describes approach to object recognition using local features where
...
detector and descriptor. Lowe further describes a system that lets
...
Consider the vector $\pth{ m_{01}, m_{10} }^T$. The angle of this vector
...
A more detailed theoretical treatment of ORB needs to be deferred until the
publication of the corresponding authoritative paper. The description about ORB
given here was based on knowledge extracted from a presentation at the {\it
International Conference on Robotics and Automation} (ICRA) 2011 in Shanghai, the ORB
implementation from OpenCV trunk, and inference from FAST and BRIEF. It is thus
...
between binary features is faster than computing the L2-norm for real-valued feature
descriptors \cite{Calonder2010}. This holds especially true when using the {\tt POPCNT}
instruction, available in AMD and Intel architectures \cite{AMD2009, Intel2007}.
...
that fits a parametrized model to given data. It is robust to outliers.  The
...
(ROC) can be used to evaluate standard two-class classifiers \cite{Fawcett2006,
...
and not by a multi-label set.  Given a finite set of labels, the number of
...
decision process. This leads to the question of how to find a parameter set
