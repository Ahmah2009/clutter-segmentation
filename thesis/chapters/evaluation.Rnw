...
the validation scenes. It achieved an average scene score of
\Sexpr{round(sps$value, 2)}.  The success rate is over-optimistic with regard
to test scenes, though, because the validation set was used for optimizing the
system. For ground truth was available for the validation set, more precise data can be presented on
it than for the test set. 
