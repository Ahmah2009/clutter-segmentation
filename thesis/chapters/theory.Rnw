...
nearest neighbor search, which permits to quickly establish correspondences
...
coordinate system}, on the other hand, is attached to the object. Both
coordinate systems are shown in \refFigure{figure:coordinate-frames}. The PRT
between object coordinates and view coordinates is called the {\it object-view
