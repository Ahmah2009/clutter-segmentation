...
\cite{Fawcett2006}. In case of multi-label classification, it is possible to
define similar metrics \cite{Tsoumakas2007}.  Unfortunately, even if object
pose were not an issue, assuming objects to appear uniquely on an image might
be unrealistic. For example, a shopping bag as in the application scenario of
this work is best represented by a multiset, and not by a multi-label set.
Given a finite set of labels, the number of multi-label sets is finite, the
number of multisets is not. A problem with infinite number of decisions is more
a problem of estimation than classification \cite{Melsa1978}. For example,
trying to define a specificity metric on it is difficult, since the number of
negatives in a multi-set is generally infinite.
