...
% TODO:
% the improvements we added to the existing libraries.
% It finally explains how system parameters were found with the help
...
The depth images are the basic units processed in a pipeline that comprises
three stages for each view.  First, the computation of the object-view
...
in some experiments but then replaced it in favor of Oriented BRIEF from
\opencv.
