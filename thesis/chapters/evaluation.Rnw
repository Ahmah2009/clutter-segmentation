...
<<>>=
    sps_avg = dbGetQuery(con, paste(
          ...
          "from experiment e",
          "join response r on e.response_id = r.id",
          "and (e.batch='run-16' or e.id = 4180)")) # FIXME:
...
camera. This bad estimate was almost two orders of magnitude off the ground truth.
...
could be pruned away by the a-priori assumption that objects are within a
certain range. Finally, radius $r$ could be replaced by a function that depends
