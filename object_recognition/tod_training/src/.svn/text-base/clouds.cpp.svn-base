...
void drawProjectedPoints(const Camera& camera, const Cloud& cloud, cv::Mat& projImg, int radius, int thickness)
{
  ...
  vector<Point2f>::const_iterator it = pts.begin();
  for (; it != pts.end(); ++it)
  {
...
Cloud createOneToOneCloud(const Features2d& f2d)
{
  ...
  {
    cloud[i] = Point3f(f2d.keypoints[i].pt.x, f2d.keypoints[i].pt.y, 0);
  }
  return cloud;
}
...
{
  Mat_<int> dense_map = projector_->project(cloud);
  Cloud _wcloud;
...
bool checkNan(const Point3f& p)
{
  return isnan(p.x) || isnan(p.y) || isnan(p.z);
}
bool checkNan(const Point2f& p)
{
...
void filterCloudNan(Cloud& cloud)
{
  ...
  {
    if (isnan(cloud[i].x) || isnan(cloud[i].z) || isnan(cloud[i].y))
    {
...
void filterCloudNan(Cloud& cloud, Features2d& f2d)
{
  ...
  for (size_t i = 0; i < cloud.size(); i++)
  {
    if (checkNan(cloud[i]))
    {
      swap(f2d.keypoints[i],f2d.keypoints.back());
      Mat row = f2d.descriptors.row(i);
...
cv::Mat_<int> CameraProjector::project(const Cloud& cloud) const
{
  ...
  Rect zrect = Rect(Point(resolution, resolution), Size(z_map.size().width - 2 * resolution, z_map.size().height - 2
      * resolution));

  vector<Point2f> points(cloud.size());
  ...
  for (size_t i = 0; i < points.size(); i++)
  {
    ...
    const Point3f& pc = cloud[i];

    if (checkNan(pc) || checkNan(p))
      continue;
    Point pi(p.x + 0.5, p.y + 0.5);

    Rect roi(p.x - resolution / 2.0f, p.y - resolution / 2.0f, resolution, resolution);
