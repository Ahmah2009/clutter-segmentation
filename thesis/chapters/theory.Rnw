...
A {\it rigid transformation} is defined as an orthogonal
...
rotation \footnote{The definitions of rigid transformations found in the
literature are inconsistent. Reflections are excluded from rigid
transformations in \cite{Forsyth2003}, but included in \cite{Galarza2007}.} .
In three dimensions, a proper rigid transformation of vectors can be
...
object coordinates and view coordinates is called the {\it object-view
transformation} and is denoted by $\oTv$. Subsequently, its inverse PRT is
called {\it view-object transformation} and denoted by $\vTo$
...
in an image. A multitude of different methods is available, but they all aim at
choosing keypoints that are repeatable for the same object over different
images. A 2D local feature consists of a keypoint and a feature descriptor that
captures information about its local image neighbourhood. Feature descriptors
are often vectors and their distance in terms of a vector norm provides a means
for measuring the similarity between two features. 
...
Given a query image, local features are extracted and matched against the
model.  This approach works well, if the object in the query scene has
approximately the same orientation as in the template view. Lowe reports that
using SIFT features, discussed in \refSection{subsection:SIFT}, it is possible
...
Notably, the strategy on how to select which features extracted from the
template views are incorporated into the model has been subject of research
...
images. Thus, repeatability is very important when trying to find
...
be addressed by a feature detector to achieve repeatability.
...
vector. Ideally, two corresponding keypoints should have very similar
...
candidate keypoints. The generated features are neither orientation nor
rotation invariant.

\subsubsection{BRIEF}

BRIEF generates binary feature descriptors for a given set of keypoints. The
BRIEF descriptor vectors depend on rotation. They are formed by a bit string
computed from the image neighbourhood of a given keypoint. Each bit stores the
result of comparing pixel intensities between two 2D points in the
neighbourhood, which are called {\it test locations}. The test locations are
initially sampled from a 2D Gaussian distribution once, and together form a
{\it test pattern} \cite{Calonder2010}. Finding nearest neighbours of binary
descriptors according to their Hamming distance is faster than for real-valued
features using the L2 norm.
...
Consider the vector $\pth{ m_{01}, m_{10} }^T \in \real^2$. The angle of this vector
...
BRIEF vectors obtained from rotated test patterns. Such ia correlation reduces
the entropy of computed BRIEF descriptors. The solution is to compute test
patterns that have only little correlation.
...
distance metric. If exactness is not required, that is approximate nearest neighbours
are sufficient, then speed-ups can be achieved. % TODO: cite
...
especially true when using the {\tt POPCNT} instruction, available in AMD and
Intel architectures \cite{AMD2009, Intel2007}.
...
nearest neighbours for binary features. LSH has applications in object
recognition, image retrieval, music retrieval, and identification of duplicates
\cite{Slaney2008}.

\subsection{Pose Estimation}
\label{subsection:pose-estimation}

% TODO: Revamp introductory sentences for pose estimation

This section shows that estimating the pose of an object can be reduced to the
perspective-$n$-point (PnP) problem, which can be solved by RANSAC even in the
...
that fits a parametrized model to given data. It is robust with respect to
...
\cite{Fischler1981}.  Generally, this algorithm randomly chooses a small set of
data points that fully determine the model parameters.  Then it forms a
...
Choosing good parameters for parametrized algorithms is a very general problem
