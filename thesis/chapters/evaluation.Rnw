...
Unfortunately, a visualization of the extracted 3D models revealed a systematic error.
The visualization is correct because it works for the 3D models from the tutorials.
The modelbases as used for the evaluation were corrupted by a systematic error of unknown
source. \refFigure{figure:systematic-error-model} shows the point cloud visualizations of
{\it downy}, an object from the \tod tutorial (a), and of the \icedtea model we created (b-c).
% FIXME:
\begin{figure}
    \begin{center}
