...
object's surface, form the {\it \glspl{model feature}}. The model features make up the
...
grayscale {\it \gls{query image}} depicting the query scene. These features are
...
image plane best explain the observed data.  Finally, if an {\it \gls{aligned model}}
