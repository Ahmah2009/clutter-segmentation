...
from the validation set. None of the three detectors runs in parallel, and the
...
the depth image. Despite of the issues presented in this section, the performance on the
validation set and the live test is promising. It remains to see whether better models can
