...
const std::vector<cv::Point2f> & Guess::projected_points() const
{
  if (projected_points_.empty())
  {
    ...
    std::vector<Point2f> & projected_points = const_cast<std::vector<Point2f> &> (projected_points_);
    if (!image_indices_.empty())
    {
      cv::Mat rvec;
    ...
    else
    {
        ...
        cv::Mat rvec;
        cv::Rodrigues(aligned_pose_.r<cv::Mat> (), rvec);
        cv::projectPoints(Mat(rotatedPoints), rvec, aligned_pose_.t<cv::Mat> (), k_, d_, projected_points);
    }
...
void Guess::draw(cv::Mat& out, int flags, const std::string& directory) const
{
  if (flags == 0)
  {
    ...
    else
    {
      ...
      cv::Rodrigues(aligned_pose_.r<cv::Mat>(), rvec);
      project3dPoints(Mat(aligned_points_), rvec, tvec, rotatedPoints);
      Tools::filterOutlierPoints(rotatedPoints, 0.9);
      if (rotatedPoints.size() < 7)
        return;
      //I think we can use _k and _d matrices here
      //projectPoints(Mat(rotatedPoints), Mat(3, 1, CV_64F, Scalar(0.0)), Mat(3, 1, CV_64F, Scalar(0.0)),
                    //k_, d_, projectedPoints);
      projectPoints(Mat(rotatedPoints), Mat(3, 1, CV_64F, Scalar(0.0)), Mat(3, 1, CV_64F, Scalar(0.0)),
      		 object_->observations[cloudIndex].camera().K, object_->observations[cloudIndex].camera().D,
      		projectedPoints);
    }
  ...
  else if (flags == 1)
  {
    if (!image_indices_.empty())
    {
      ...
      for (size_t i = 0; i < aligned_points_.size(); i++)
      {
        ...
        kpts_query.push_back(KeyPoint(image_points_[i].x, image_points_[i].y, 0, 0, 0, 0));
      }
                                      ...
                                      std::vector<Guess>& guesses, boost::dynamic_bitset<> & is_interesting)
{
  ...
  while (true)
  {
    ...
    std::vector<GuessMatch> ordered_matches;
                                      ...
                                      vector<Guess>& guesses)
{
  ...
  Features2d f2d;
  f2d.keypoints = keypoints;
  f2d.image = image;
  
  //temporary returned assumption about the parameters of the test camera
  f2d.camera = object->observations[0].features().camera;
  ...
  foreach(const vector<int>& cluster, clusterIndices)
        {
          ...
          calculateGuesses(f2d, empty_cloud, object, matches, guesses, is_interesting);
        }
}
