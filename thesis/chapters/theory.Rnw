...
The {\it \gls{camera coordinate system}} is attached to the camera. Two of its basis
vectors define the image plane and the remaining basis vector is directed
towards the scene. The camera coordinate system is convenient for the robot. For
example, if the robot is programmed to grasp an object at two points, it needs
to know the camera coordinates of the two grasping points. The {\it \gls{object
...
object coordinates and camera coordinates is called the {\it \gls{object-camera
transformation}} and is denoted by $\ocT$. Subsequently, its inverse PRT is
called {\it \gls{camera-object transformation}} and denoted by $\coT$
...
definitions given previously in \refSection{section:object-camera-transformations},
