...
validation set, % on a test set
and results from a live test on the PR2 robot at Technische Universität
München. It covers how the modelbase was built, how validation data, ground
truth, and the test data have been obtained. We discuss the parameter set found
by experimentation. Scenes are presented in which the system performs well.  We
also cover the limitations and issues that were revealed during the evaluation.

\section{Data Acquisition}
\label{section:data-acquisition}

This section shows how all the data required for constructing the modelbases and
the validation set
% and the test set
was obtained. The modelbases and the validation set required raw data, which
have been collected in a similar fashion and the same hardware and software
setup.
...
per second. The camera matrix needs to be recorded, as it is required at least
in solving the point-to-point correspondence problem in pose estimation. 
...
object-camera transformation for each view, and in masking for each view as
described in \refSection{subsection:learning}.  These post-processed raw data
provide the masks and the object-camera transformation which can be used to
...
\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.5\textwidth]{modelbase}
...
% \begin{figure}
%    \begin{center}
%        \includegraphics[width=0.3\textwidth]{modelbase-3D}
...
scenes.  We recorded around 50--60 views of various scenes, each containing
three objects from the modelbase. Out of those, we chose 21 scenes to form the
validation set. We chose them in roughly equal angle increments such that the
variance in viewpoint between two scenes is maximized. The scenes show the
objects with varying levels of occlusion. Each object appears in upright
orientation.

\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.8\textwidth]{validation-scene}
...
Given a scene, ground truth requires the computation of the object-camera transformations
$\ocTi{i}$, $\ocTi{j}$, and $\ocTi{k}$ for the three objects $i$, $j$ and $k$
...
\refFigure{figure:validation-scene}. We placed object $j$ in the centre between
the fiducial markers, such that 

\begin{equation}
    \ocTi{j}: \vec{p} \mapsto\ \fcT \pth{\vec{p}}
\end{equation}
...
\begin{equation}
    \ocTi{i}: \vec{p} \mapsto\ \fcT \pth{ \vec{p} - \begin{pmatrix} 0.15 \\ 0 \\ 0 \end{pmatrix} }
\end{equation}

\begin{equation}
    \ocTi{k}: \vec{p} \mapsto\ \fcT \pth{ \vec{p} + \begin{pmatrix} 0.13 \\ 0 \\ 0 \end{pmatrix} }
\end{equation}
...
@
    ...
    \caption[The observed success rates in the experiments]{From left to right:
         (a) the smoothed scatter plot of success rates of \Sexpr{e_all$cnt} experiments in total.
We assigned identifiers to the experiments in increasing order with time. The patterns reflect our
tactics in choosing which part of parameter space to explore next.
(b) The observed distribution of
success rates in all experiments. Bad parameter values result in a zero success rate.
...
parameters {\tt octaves} and {\tt scale\_factor}. We chose the value for
{accept\_threshold} empirically such that it exceeded the number of inliers in
cases where a mislabelling was observed.
...
stages to match the binary features produced by ORB. 
...
The performance of the \clutseg system has been tested on the validation set,
% on a separate test set,
and in a live test on the PR2 robot. This section reviews \clutseg's
performance for a parameter set that worked well on the validation set. 
% It also covers observations made on a broad range of experiments.

\subsection{Performance on Validation Set}
\label{subsection:performance-validation-set}
...
Since \clutseg uses randomized algorithms, we have run it with the SPS values
for 20 times on the validation set to measure the average performance.  The
system correctly recognized an object within error bounds of 3 cm and 20
...
labelled axes, the estimated pose, and the keypoints of query features that
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{validation-scene-success}
...
<<>>=
    sps_bf_avg = dbGetQuery(con, paste(
          "select e.id as id,",
          "avg(r.succ_rate) as avg_succ_rate,",
          "avg(r.value) as avg_value,",
          "avg(r.none_rate) as avg_none_rate,",
...
Recognizing an object took \Sexpr{signif(sps_avg$avg_test_runtime / 21, 2)}
seconds on average using the setup described in
\refTable{table:hardsoftware-parameter-optimization}. Using brute-force nearest
neighbour search instead of Locality Sensitive Hashing, the system took
\Sexpr{signif(sps_bf_avg$avg_test_runtime / 21, 2)} seconds per image.
Brute-force search yielded \Sexpr{round(sps_bf_avg$avg_detect_matches)}
correspondences per image, very similar to LSH. Since \tod ensures injectivity
in the mapping of query features to model features, and since LSH does not
guarantee to find $k$ nearest neighbours, the number of correspondences found
by either LSH or brute-force matching does not need to match. So, LSH ran at
least a factor \Sexpr{floor(sps_bf_avg$avg_test_runtime / sps_avg$avg_test_runtime)}
faster than brute-force search. When measuring only
the runtime for feature matching alone, we observed LSH to be more than a
magnitude faster than brute-force search. The success rate of LSH on the 
validation set was \Sexpr{round(100 * sps_bf_avg$avg_succ_rate)}\%.

<<>>=
    rm(e_orb)
@

For the selected parameter values, the success rate of \Sexpr{round(100 * sps_avg$avg_succ_rate)}\% is over-optimistic with regard to the test scenes,
because the validation set was used for optimizing the system. For ground truth
was available for the validation set, more precise data can be presented on it
than
% for the test set. 
for the live test.
...
This section shows the results of a live test of \clutseg on the PR2. We tested
whether the system is able to recognize an object in cluttered scenes with high
occlusion rates and objects in arbitrary orientation
(\refFigure{figure:clutter}); how duplicates in the scene affect the
recognition process (\refFigure{figure:duplicates}); in which setups \clutseg
confuses objects (\refFigure{figure:confusion}); and whether the system is able
to recognize an object in the presence of many textured objects in the scene
(\refFigure{figure:many}).  Finally, we tested whether the PR2 is able to grasp
a recognized object (\refFigure{figure:grasp}).

The guess refinement only matches query features against the model features of
one single object (\refSection{subsection:guess-refinement}). The figures in
this section show the number of correspondences (inliers) consistent with the
estimated pose, and the total number of correspondences between the query
image and the single model. For example, consider the guess with label {\it
\haltbareMilch(10, 201)} from \refFigure{figure:clutter} (a). Its consensus set
contains $10$ inliers. Between the query image and the model of \haltbareMilch,
201 correspondences were found in total.

% uprightness

Recognition worked better when the objects were in upright position. \clutseg
had more difficulties with objects in  arbitrary orientations.
\refFigure{figure:clutter} (a) shows a scene where \haltbareMilch was
recognized. Only 10 inliers were consistent with its estimated pose, which is
also off the ground truth. \refFigure{figure:clutter} (b) presents a similar
scene where \icedtea was accurately recognized.

In scenes containing \icedtea and at least one other object from the modelbase,
we observed that \clutseg has a strong tendency to recognize \icedtea. We
tested each of the six possible pairings of the four objects in the modelbase.
The relation, where $A > B$ denotes ``A is recognized if shown together with B
in a scene'' was observed to be \icedtea > \haltbareMilch = \jacobsCoffee >
\assamTea. The four objects are described by a different number of model
features (\refTable{table:number-model-features}). From the initial guesses,
\clutseg selects the one with the highest guess ranking, which explains this
observation.
...
\begin{table}
    \begin{center}
        \begin{tabular}{lr}
            ...
            \jacobsCoffee   & 8912 \\
        \end{tabular}
        \caption[The numbers of model features for the four objects in the modelbase]
        {The number of model features of \assamTea, \haltbareMilch, \icedtea, and \jacobsCoffee.}
        \label{table:number-model-features}
    \end{center}
\end{table}

\clutseg and \tod do not perform any clustering of correspondences prior to
pose estimation. With duplicates on the scene, the number of inliers consistent
with the estimated pose of an object decreases. This can be explained by \tod
making the mapping from query features to model features injective. Yet, even
in the presence of many duplicate objects, \clutseg still recognizes objects
(\refFigure{figure:duplicates}).

Some shopping items do not only resemble each other in shape, but also in
appearance. The (wild-berries) \icedtea and the peach-flavored ice tea were
confused by \clutseg (\refFigure{figure:confusion} a). \clutseg guessed
correctly after adding (wild-berries) \icedtea to the scene
(\refFigure{figure:confusion} b).

The recognition of objects is generally rendered more difficult when many
textured object are on the scene; it is more likely that the nearest neighbour
of a query feature is not a true correspondence between object and model. How
difficult it becomes is an open question. We tested scenes with many textured
objects. Most of them did not belong to the modelbase though. \clutseg
encountered no difficulties in the scenes depicted by \refFigure{figure:many}
nor in similar scenes tested.

The grasping algorithm is based on computing the centroid of the inlier cloud.
The gripper of the PR2 is unfortunately not large enough to grasp \icedtea.
The maximum aperture of the gripper permits the PR2 to grasp \assamTea,
\haltbareMilch and \jacobsCoffee, but there is almost no margin for error.
This made the grasping in the live test hard. However, in all test runs, we
observed the PR2 to correctly steer its manipulator correctly towards the
object.  A successful attempt where the PR2 grasps the recognized \assamTea is
shown in \refFigure{figure:grasp}.
...
% the images taken were recorded from a Kinect camera in 1280x1024 resolution

% the more surface the PR2 sees, the easier recognition

% the vertical angle affects the recognition process

% angles closer to the angle in the training data work better 

% if objects do not stand upright on the table, recognition is more difficult
% and often, pose is not correctly estimated

% the inliers almost always belong only to the guessed object


% duplicates do not severely affect the recognition process,
% but they affect the number of inliers

% many textured objects render recognition more difficult
% the number of inliers decreases if more objects are put in the scene
...
% the grasping is centroid-based

% push-grasping would be a enhancement

% the PR2 gripper aperture is not wide enough to grasp the objects reliably

% MISCELLANEOUS

% recognition is pretty fast, about three seconds

% icedtea is the easiest object to recognize

% assam tea is the hardest object to recognize

% haltbareMilch easier than assamtea


% icedtea easier than assamtea


% icedtea easier than haltbaremilch


% icedtea easier than jacobscoffee


% jacobscoffee easier than assamtea
% haltbareMilch and jacobscoffee equal 


\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{clutter}
        \caption[\clutseg recognizes \haltbareMilch and \icedtea in cluttered
scenes]{Two cluttered scenes with all four objects from the model base as seen
by the PR2. From left to right: (a) \haltbareMilch was recognized with 10
inliers out of 201 correspondences between the image and the model of
\haltbareMilch. The pose is fairly off the ground truth pose, yet the inliers
belong to the right object. (b) \clutseg precisely estimated the pose of
\icedtea with a confidence value of 155 inliers out of 516 correspondences in a
scene with high occlusion rate.}
        \label{figure:clutter}
    \end{center}
\end{figure}

\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{duplicates} \caption[\clutseg
recognizes \icedtea and \assamTea in scenes with duplicates]{From left to
right: (a) \icedtea was recognized despite of the presence of many duplicates.
(b) \clutseg recognized one of the four items of \assamTea.}
\label{figure:duplicates}
    \end{center}
\end{figure}
...
\begin{figure}
    \begin{center}
        ...
        \caption[\clutseg confuses peach ice tea with \icedtea]{From left to
right: (a) The (wild berries) \icedtea in the modelbase was confused with peach
ice tea. (b) The additional presence of (wild berries) \icedtea resolves the
confusion. }
...
\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{many}
\caption[\clutseg recognizes \icedtea and \jacobsCoffee in scenes with
many objects]{Many textured objects in the scene render recognition
more difficult. From left to right: (a) \icedtea was recognized with 40 inliers
in a scene with 11 textured objects. (b) After removing \icedtea and its
easy-to-confuse peach-flavored variant, \jacobsCoffee was recognized with 21
inliers out of 284 correspondences.}
    \label{figure:many}
    \end{center}
\end{figure}
...
\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{grasp}
        \caption[The PR2 grasps \assamTea]{The PR2 grasps \assamTea.}
        \label{figure:grasp}
    \end{center}
\end{figure}
...
\subsection{ORB}
\label{subsection:evaluation-orb}

This subsection discusses results of experiments with ORB. It covers
...
from the validation set. None of the three detectors run in parallel, and the
...
\cite{Tuytelaars2007}. ORB has such a threshold, and our evaluation 
indicates that this threshold is effective.

The parameter {\tt n\_features} specifies the number of desired features
produced by ORB. Other feature detectors and descriptors in \opencv, such as
SIFT and SURF do not provide such a parameter. Neither does the FAST feature
detector. The parameter {\it n\_features} is simple and intuitive. Yet,
ORB does not make any guarantee about the actual number of features extracted
on the image. The parameter is just a hint. The question arises how well ORB
responds to different numbers of desired features.  ORB selects features based
on the Harris corner measure \cite{Rublee2011}.

\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{orb-response-test-images}
        \caption[Four test images for ORB]{From left to right: (a)
...
@
     ...
     \caption[The response of ORB on the number of desired features]
         {The response of ORB on the number of desired features.}
...
The system had difficulties to recognize \icedtea when shown from a certain
angle.  It turned out that \icedtea has smallprint almost all over its backside
(\refFigure{figure:icedtea-smallprint}). The same set of letters repeatedly
appear over a large area, which probably makes it hard to obtain informative
features. Furthermore, focus becomes an issue.

\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.2\textwidth]{icedtea_smallprint}
        \caption[Repetitive texture shown on the back of \icedtea]{Repetitive texture on template object \icedtea.}
        \label{figure:icedtea-smallprint}
    \end{center}
\end{figure}
...
keypoint $q_i$ and a model feature with 3D point $p_i$. Let $\ocTh$ be a
...
model point $p_i$, aligned to the scene by $\ocTh$, as

\begin{equation}
    \hat{q}_i := U\pth{ \ \ocTh \pth{ p_i}\ }
\end{equation}
...
\begin{equation}
    c_i\ \text{ is an inlier w.r.t. }\ \ocTh\quad \Leftrightarrow \quad \normtwo{\ q_i - \hat{q}_i\ } \le r  \quad \wedge \quad \text{$p_i$ is visible}
    \label{equation:inlier-definition}
\end{equation}

Now, let us model the scenario, where a distant pose estimate was generated
for an object that is close to the camera in truth. Let $\ocT$ denote
ground truth for $\ocTh$. 
 
\begin{equation}
    e_t \pth{ \ocTh,\ \ocT } \geq l
\end{equation}
...
\begin{equation}
    \forall{i \in \brt{n}}:\quad c_i\ \text{ is an inlier w.r.t. }\ \ocTh\quad \Leftrightarrow \quad \normtwo{\ q_i - \hat{q}\ } \le r \quad \wedge \quad \text{$p_i$ is visible}
\end{equation}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.55\textwidth]{far-away-aligned-model}
...
\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.4\textwidth]{ransac-max-projection-error-deficiency-sample}
...
\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.8\textwidth]{systematic-error-model}
...
\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.9\textwidth]{clutseg-model-xyhist}
...
\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.9\textwidth]{tod-model-xyhist}
...
the depth image. Despite the issues presented in this section, the performance
on the validation set and the live test is promising. It remains to see whether
better models can further improve results.
