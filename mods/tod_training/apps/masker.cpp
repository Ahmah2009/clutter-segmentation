...
namespace {
    ...
    int options(int ac, char **av, masker_options & opts)
    {
        ...
        desc.add_options()("mask_type,M",
                           po::value <
                           int >(&opts.mask_type)->default_value(0),
...
struct masker_worker
{
    ...
    masker_worker(const std::string & directory, const Camera & camera,
                  const list < string > &images, const list < string > &pcds,
                  const Masker * masker,
                  const masker_options & opts, masker_stats & stats) : directory(directory),
        images(images), pcds(pcds), masker(masker), camera(camera), opts(opts), stats(stats)
    {
    ...
    void operator() ()
    {
        if (!pcds.empty() && pcds.size() != images.size()) {
            throw std::
                runtime_error
                ("pcd_list and image_list need to be the same size!");
        }
        ...
        foreach(const std::string & x, images)
        {
            ...
            try {
                ...
                std::
                    cerr << "[MASKER] FAILED to mask (unknown pose): " << x <<
                    std::endl;
                sync(stats_lock, stats.failure_cnt++);
...
boost::mutex masker_worker::stats_lock;
...
int main(int argc, char *argv[])
{
    ...
    switch (opts.mask_type) {
        case 0:
            {
                ...
                {
                    try {
                        fiducial::KnownPoseEstimator pose_est(opts.common.
                                                              directory +
                                                              "/" + x +
                                                              ".pose.yaml");
                        cv::Mat color =
                            imread(opts.common.directory + "/" + x);
                        Features2d f2d(camera, color);
                        f2d.camera.pose = pose_est.estimatePose(Mat());
        ...
        case 1:
            ...
            if (pcds.size() != images.size()) {
                throw std::
                    runtime_error
                    ("must have the same number of point clouds as images");
            }
    ...
    std::vector < std::list < std::string > >ilist =
    ...
    for (size_t i = 0; i < ilist.size(); i++) {
        {
            threads.
                create_thread(masker_worker
                              (opts.common.directory, camera, ilist[i],
                               pcds.empty()? pcds : plist[i], &*masker,
                               opts, stats));
        }
    }
