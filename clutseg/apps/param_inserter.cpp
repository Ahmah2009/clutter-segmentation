...
void cmd_insert(sqlite3* & db) {
    ...
    input.refine_pms_file = bfs::path(readvalue("refine_pms_file", defaults.refine_pms_file.string()));
    ...
    TODParameters refine_pms;
    readTodParams(input.refine_pms_file, refine_pms);
    ...
    exp.paramset.refine_pms_match = refine_pms.matcherParams;
    exp.paramset.refine_pms_guess = refine_pms.guessParams;
