...
<<echo=FALSE>>=
    ...
    e_orb = dbGetQuery(con, paste(
          ...
          "join pms_fe r on p.recog_pms_fe_id = r.id",
          "where e.response_id not null",
          "and t.detector_type = 'ORB-OpenCV'",
          "and t.descriptor_type = 'ORB-OpenCV'",
          "and t.extractor_type = 'ORB-OpenCV'",
          "and r.detector_type = 'ORB-OpenCV'",
          "and r.descriptor_type = 'ORB-OpenCV'",
          "and r.extractor_type = 'ORB-OpenCV'"))

    # TODO: solve problem with ORB and ORB-OpenCV
@

% TODO: introduce rBRIEF
% TODO: introduce ORB-OpenCV and tell the difference from ORB
...
The experiments took more than \Sexpr{floor(e_all$total_test_runtime / 3600)}~hours
of wall-time to compute on This is an average of
\Sexpr{floor(10 * e_all$avg_test_runtime / 60) / 10}~minutes per experiment.
...
<<echo=false>>=
    sps = dbGetQuery(con, paste(
          ...
          "where tf.detector_type = 'ORB-OpenCV'",
          "and tf.descriptor_type = 'ORB-OpenCV'",
          "and tf.extractor_type = 'ORB-OpenCV'",
