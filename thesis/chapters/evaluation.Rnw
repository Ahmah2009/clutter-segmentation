...
masking for each view as described in Section \ref{subsection:learning}.  This
...
Figure~\ref{figure:modelbase} shows the four different household objects for
...
Figure~\ref{figure:modelbase-3D} shows the model features of a selected
...
A practical consequence of Equation~\ref{equation:fiducial-object} is that a
...
\subsection{Computation Speed of Oriented BRIEF}

This section discusses a case, where Oriented BRIEF outperformed SIFT and SURF
in speed of computation, while producing keypoints in greater quantities. We
made this observation on the validation set.

For SIFT, SURF and ORB with the default parameters specified in OpenCV revision
TODO, Figure~\ref{figure:sift-surf-orb-benchmark} shows three statistics, averaged
over 21 images. First, the average clock time required to extract features per
image. Second, the average clock time required per keypoint and image. Third,
the average number of keypoints extracted per image.
...
<<echo=false>>=
    ...
    barplot(d$time_img, names.arg=d$extractor, main="Time per image",    ylab="time [s]",  col=cols)
    barplot(d$time_kpt, names.arg=d$extractor, main="Time per keypoint", ylab="time [ms]", col=cols)
    barplot(d$quantity, names.arg=d$extractor, main="Keypoint quantity", ylab="quantity",  col=cols)
    invisible(dev.off())
@
     \includegraphics[width=0.6\textwidth]{media/feature-extractor-performance}
     \caption{Benchmark results of SIFT, SURF and ORB}
     \label{figure:sift-surf-orb-benchmark}
    \end{center}
\end{figure}

Oriented BRIEF ran about X times faster than SIFT, and about Y times faster than SURF.
...
(q_i, p_i) \in \mathbb{R}^2 \times \mathbb{R}^3 $ between a query feature with
keypoint $q_i$ and a model feature with 3D point $p_i$. Let $_o\hat{T}^v$ be a
pose estimate. Let $U: \mathbb{R}^3 \to \mathbb{R}^2$ denote perspective
projection for the calibrated camera. Define the projection $\hat{q}_i$ of the
model point $p_i$, aligned to the scene by $_o\hat{T}^v$, as

\begin{equation}
    \hat{q}_i := U\left(\ _o\hat{T}^v\left(p_i\right)\right)
\end{equation}
...
\begin{equation}
    c_i\ \text{ is an inlier w.r.t. }\ _o\hat{T}^v\quad \Leftrightarrow \quad \normtwo{\ q_i - \hat{q}_i\ } \le r
    \label{equation:inlier-definition}
\end{equation}

Now, let us model the scenario, where a distant pose estimate was generated
for an object that is close to the camera in truth. Let $_o\hat{T}^v$ denote
ground truth for $_o\hat{T}^v$. 
 
    % \normtwo{_o\hat{T}^v\left(\vec{0}\right) - _oT^v\left(\vec{0}\right)}
\begin{equation}
    e_t\left(_o\hat{T}^v,\ _oT^v\right) \geq l
\end{equation}
...
\begin{equation}
    \forall{i \in \left[n\right]}: \hat{q}_i \approx \hat{q}
    \label{equation:same-point}
\end{equation} 

Plugging Equation~\ref{equation:same-point} into Equation~\ref{equation:inlier-definition},
we obtain an inlier criteria for the bad estimate

\begin{equation}
    \forall{i \in \left[n\right]}:\quad c_i\ \text{ is an inlier w.r.t. }\ _o\hat{T}^v\quad \Leftrightarrow \quad \normtwo{\ q_i - \hat{q}\ } \le r
\end{equation}
...
estimated distance $\normtwo{_o\hat{T}^v\left(\vec{0}\right)}$.
