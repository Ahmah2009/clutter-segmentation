...
Since \clutseg uses randomized algorithms, we have run it with the SPS
...
Brute-force search yielded \Sexpr{round(sps_bf_avg$avg_detect_matches)} correspondences per image, very
similar to LSH. Hence, LSH ran at least a factor
\Sexpr{floor(sps_bf_avg$avg_test_runtime / sps_avg$avg_test_runtime)}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{clutseg-model-xyhist}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{tod-model-xyhist}
