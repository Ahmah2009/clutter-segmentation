...
produce a set of guesses $D$ for all objects in the model base. A ranking
defines a total order on $D$. The inlier ranking $r_S$ is used for this by
...
was made at all, the algorithm proceeds to the next guess in $D$ and repeats the
...
In this section, the parameters of the \clutseg system are reviewed, their 
nature discussed and the approach explained that has been used to find a
reasonable parameter set. Statistics are introduced that measure performance
of the \clutseg system on sets of query scenes where ground truth is available.
...
\begin{table}[h!]
    ...
    \end{tabular}
    \caption{\clutseg parameters in matching and recognition}
...
Feature extraction parameters control the extraction of local 2-d features.  A
...
chapter \ref{chapter:introduction} call for a feature detector that produces
...
refine\_max\_projection\_error}). No further pose estimates will be generated
as soon as RANSAC returns an estimate with a consensus set of size less than
{\tt detect\_min\_inliers\_count} ({\tt refine\_min\_inliers\_count}).
...
model-view transformation failed due to undetected checkerboards is tracked.
The time required for building the model base is recorded as well. Results are
presented in chapter \ref{chapter:evaluation}.
...
\begin{equation}
    G = \left\{ (i, T) : \text{object $i$ is on scene at pose defined by MVT T} \right\}
\end{equation}

% TODO: introduce abbreviation for model-view transformation
% TODO: object-view transformation or model-view transformation, consistent with object coordinates or model coordinates
...
and {\tt none\_rate} captures these cases, averaged over multiple query scenes.
...
ground truth pose $T$ and pose estimate $\hat{T}$ is

\begin{equation}
    e_t = \left|\ \hat{T}\left(\vec{0}\right) - T\left(\vec{0}\right)\ \right|
\end{equation}
...
\begin{equation}
    e_\alpha = \left| \arccos \left( \frac{\vec{r}^t\ \hat{\vec{r}}}{\normtwo{\vec{r}}\ \normtwo{\hat{\vec{r}}}} \right) \right|
\end{equation}
...
Here, a scene score is presented that measures \clutseg's performance on a
single query scene. The goal is to find a score function that closely models
the utility of a guess made for a query scene.
...
query scene with ground truth $G$ and a guess $g$, the scene score function $u$
is defined as
...
\clutseg collects statistics about the recognition process that do not involve
...
The strategy resembled genetic programming.
...
sets when idle. New parameter sets are tested with computationally cheap tests
for validity and those leading to errors are marked and skipped, such that one
failing experiment does not kill the process. The primary purpose of the
database was to collect experiment inputs and outputs in a central, accessible
and convenient location.  No efforts have been spent on normalization,
performance tuning or any other aspects that were not related to its primary
purpose. For convenience, data can be selected from various database views.
...
up around 5 Megabytes on the filesystem. \tod stores the modelbase as
compressed YAML text files. If disk space and reading the modelbase quickly
into memory were an issue, then binary files would be a good choice.
