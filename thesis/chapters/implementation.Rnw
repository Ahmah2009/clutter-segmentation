...
% TODO: maybe include input and output picture
...
camera and by its observed color.
...
in some experiments but we then replaced in favor of ORB from \opencv.
...
have not tested. \tod offers a brute-force matching algorithm which we use in
...
and \ref{subsection:measured-statistics}, respecitvely. The experiment runner
